...
const Dropdown = ({ setDuration, duration }) => {
    ...
    return (
        <div ref={dropdownRef} className="">
            ...
            tracking-wider border-2 border-transparent active:border-pink duration-200 active:text-[#FD00FE]"> {duration || "Duration"}
            ...
            mt-2 flex flex-col items-start rounded-lg">
                    {list.map((item, i) => (
                        <div key={i} className="flex w-full p-3 justify-between hover:bg-[#FD00FE]  cursor-pointer rounded-lg
                            border-l-transparent font-roboto-mono text-[14px] active:bg-pink-light" onClick={() => setDuration(item.time)}>
                            <h3>{item.time}</h3>
                            <h3>{item.emoticon}</h3>
                        </div>
                    ))}
